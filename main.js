...
import { createGameRecord } from './game-record.js?v=3a54e4a165';
import { createGameRecordStore } from './game-record-store.js?v=3a54e4a165';
import {
  createGameReview,
  ...
  lastGameReviewPly,
  selectGameReviewPly,
} from './game-review.js?v=3a54e4a165';
import {
...
import { createGameReviewEvidence } from './game-review-evidence.js?v=3a54e4a165';
...
} from './game-teaching-mode.js?v=3a54e4a165';
import {
...
} from './coach-model-profile-preference.js?v=3a54e4a165';
import {
...
import { createGameReviewPuzzleHandoff } from './game-review-puzzle-handoff.js?v=3a54e4a165';
import {
  createGameLiveReviewHandoff,
  consumeGameLiveReviewHandoff,
  createGameLiveReviewAnalysis,
} from './game-live-review-handoff.js?v=3a54e4a165';
import {
  PuzzleEditorError,
  createEditorState,
  placeEditorPiece,
...
} from './puzzle-editor.js?v=3a54e4a165';
import {
  PuzzleRecorderError,
  createRecorder,
  recordMove,
...
} from './puzzle-recorder.js?v=3a54e4a165';
import {
  PuzzlePracticeError,
...
} from './puzzle-practice.js?v=3a54e4a165';
import { PuzzleStoreError, createPuzzleStore } from './puzzle-store.js?v=3a54e4a165';
import {
  PracticeAnalyticsError,
  createPracticeAnalyticsStore,
} from './puzzle-analytics.js?v=3a54e4a165';
import {
  ...
  parsePuzzleImport,
} from './puzzle-transfer.js?v=3a54e4a165';
import {
  PHOTO_MAX_ZOOM,
  PHOTO_MIN_ZOOM,
  PuzzlePhotoError,
  ...
  zoomPhotoIn,
  zoomPhotoOut,
} from './puzzle-photo.js?v=3a54e4a165';
import {
...
} from './puzzle-photo-calibration.js?v=3a54e4a165';
import {
  ...
  selectionKey,
} from './puzzle-photo-recognition.js?v=3a54e4a165';
import {
  addTemplate,
...
} from './puzzle-photo-piece-types.js?v=3a54e4a165';
import {
...
} from './puzzle-photo-review.js?v=3a54e4a165';
...
let gameReviewTeachingMessageState;
let gameTeachingModeState = createGameTeachingModeState();
let gameTeachingModeWorker = null;
let gameTeachingReviewHandoff = null;
const gameReviewCoachStagingCapability = readInstalledReviewCoachStagingCapability();
const gameReviewCoachRequesterDescriptor = Object.getOwnPropertyDescriptor(
  globalThis,
  '__CHINESE_CHESS_REVIEW_COACH_REQUESTER__',
);
const gameReviewCoachMockRequester = gameReviewCoachRequesterDescriptor
  && Object.prototype.hasOwnProperty.call(gameReviewCoachRequesterDescriptor, 'value')
  && typeof gameReviewCoachRequesterDescriptor.value === 'function'
  ? gameReviewCoachRequesterDescriptor.value
  : null;
const gameReviewCoachRequester = gameReviewCoachStagingCapability?.requestCoach
  ?? gameReviewCoachMockRequester;
const gameReviewCoachCapabilitiesLoader = gameReviewCoachStagingCapability?.loadCapabilities ?? null;
let gameReviewCoachState = gameReviewCoachRequester
  ? createIdleCoachState(0, readCoachModelProfilePreference(() => globalThis.localStorage))
  : createDisabledCoachState();
let gameReviewCoachRequest = null;
let gameReviewCoachRequestSequence = 0;
let gameReviewCoachStatusMessage = '';
let gameReviewCoachCapabilitiesState = Object.freeze({
  status: gameReviewCoachCapabilitiesLoader ? 'idle' : 'not-required',
  revision: 0,
  snapshot: null,
});
let gameReviewCoachCapabilitiesRequest = null;
const gameReviewCoachUnavailableProfiles = new Set();

let aiWorker = null;
let aiModule = null;   // Worker 不可用時的主執行緒後備
try {
  aiWorker = new Worker(new URL('./ai-worker.js?v=3a54e4a165', import.meta.url), { type: 'module' });
...
function requestAIMove() {
  ...
  if (aiWorker) {
    ...
    (aiModule ??= import('./ai.js?v=3a54e4a165')).then(({ findBestMove }) => {
      setTimeout(() => {
...
function terminateGameTeachingModeWorker(worker = gameTeachingModeWorker) {
  if (!worker) return;
  worker.terminate();
  if (worker === gameTeachingModeWorker) gameTeachingModeWorker = null;
}

function renderGameTeachingMode() {
  const available = normalGameActive() && isAI();
  const message = available && gameTeachingModeState.enabled
    ? gameTeachingModeState.message
    : null;
  btnTeachingMode.disabled = !available;
  btnTeachingMode.textContent = `教學模式：${gameTeachingModeState.enabled ? '開' : '關'}`;
  btnTeachingMode.setAttribute('aria-pressed', String(gameTeachingModeState.enabled));
  gameTeachingCard.classList.toggle('hidden', !message);
  rightPanel.classList.toggle('teaching-visible', !!message);
  gameTeachingTitle.textContent = message?.title ?? '';
  gameTeachingBody.textContent = message?.body ?? '';
  gameTeachingPlayed.textContent = gameTeachingModeState.evidence?.played.notation ?? '';
  gameTeachingCandidate.textContent = gameTeachingModeState.evidence?.candidate.notation ?? '';
}

function invalidateTeachingModeFeedback() {
  terminateGameTeachingModeWorker();
  gameTeachingModeState = invalidateGameTeachingMode(gameTeachingModeState);
  gameTeachingReviewHandoff = null;
  renderGameTeachingMode();
}

function setTeachingModeEnabled(enabled) {
  if (typeof enabled !== 'boolean') return false;
  terminateGameTeachingModeWorker();
  gameTeachingModeState = setGameTeachingModeEnabled(gameTeachingModeState, enabled);
  renderGameTeachingMode();
  return true;
}

function captureTeachingModeSource(from, to) {
  if (!shouldScheduleGameTeachingMode({
    enabled: gameTeachingModeState.enabled,
  ...
  return {
    ...
    movePly: history.length + 1,
    ply: history.length,
    board: board.map((row) => row.map((piece) => (piece ? { ...piece } : null))),
    sideToMove: turn,
    repetitionHistory: repHistory.map((entry) => ({ ...entry })),
    playedMove: { from: { ...from }, to: { ...to } },
  };
}

function handleGameTeachingModeResponse(worker, response) {
  if (worker && worker !== gameTeachingModeWorker) return false;
  terminateGameTeachingModeWorker(worker);
  if (!normalGameActive() || !isAI()
    || !gameTeachingModeMatchesHistory(
      gameTeachingModeState,
      normalGameRecordSession?.id ?? '',
      history,
    )) return false;
  const settled = settleGameTeachingModeAnalysis(gameTeachingModeState, response);
  if (!settled.accepted) return false;
  gameTeachingModeState = settled.state;
  try {
    gameTeachingReviewHandoff = createGameLiveReviewHandoff({
      teachingState: gameTeachingModeState,
      session: normalGameRecordSession,
      history,
    });
  } catch {
    gameTeachingReviewHandoff = null;
...
function requestGameTeachingModeAnalysis(source) {
  if (!source || !normalGameActive() || !isAI() || !gameTeachingModeState.enabled) return false;
  let started;
  try {
    started = beginGameTeachingModeAnalysis(gameTeachingModeState, source);
  } catch {
    invalidateTeachingModeFeedback();
    return false;
  }
  if (!started.scheduled) return false;
  terminateGameTeachingModeWorker();
  gameTeachingModeState = started.state;
  renderGameTeachingMode();
  let worker;
  try {
    worker = new Worker(new URL('./ai-worker.js?v=3a54e4a165', import.meta.url), { type: 'module' });
    gameTeachingModeWorker = worker;
    worker.onmessage = (event) => handleGameTeachingModeResponse(worker, event.data);
    worker.onerror = (event) => {
      event.preventDefault?.();
      handleGameTeachingModeResponse(worker, {
        kind: 'review-candidate',
        recordId: started.request.recordId,
        ply: started.request.ply,
        revision: started.request.revision,
        error: 'Teaching analysis worker failed.',
      });
    };
    worker.postMessage(started.request);
  } catch {
    handleGameTeachingModeResponse(worker ?? gameTeachingModeWorker, {
      kind: 'review-candidate',
      recordId: started.request.recordId,
      ply: started.request.ply,
      revision: started.request.revision,
      error: 'Teaching analysis worker could not start.',
    });
  }
  return true;
}
...
window.__chess = {
  ...
  get teachingMode() {
    return {
      enabled: gameTeachingModeState.enabled,
      status: gameTeachingModeState.status,
      revision: gameTeachingModeState.revision,
      message: gameTeachingModeState.message
        ? structuredClone(gameTeachingModeState.message)
        : null,
      workerActive: !!gameTeachingModeWorker,
    };
  },
  get gameTeachingReviewHandoff() { return gameTeachingReviewHandoff; },
  openGameTeachingReviewHandoff,
  get gameReviewCoach() {
    return {
      enabled: !!gameReviewCoachRequester,
      status: gameReviewCoachState.status,
      revision: gameReviewCoachState.revision,
      requestId: gameReviewCoachState.request?.requestId ?? null,
      framing: gameReviewCoachState.framing
        ? structuredClone(gameReviewCoachState.framing)
        : null,
      requestActive: !!gameReviewCoachRequest,
      staging: !!gameReviewCoachStagingCapability,
      capabilitiesStatus: gameReviewCoachCapabilitiesState.status,
      capabilities: gameReviewCoachCapabilitiesState.snapshot
        ? structuredClone(gameReviewCoachCapabilitiesState.snapshot)
        : null,
    };
  },
...
const gameTeachingCard = document.getElementById('gameTeachingCard');
const gameTeachingTitle = document.getElementById('gameTeachingTitle');
const gameTeachingBody = document.getElementById('gameTeachingBody');
const gameTeachingPlayed = document.getElementById('gameTeachingPlayed');
const gameTeachingCandidate = document.getElementById('gameTeachingCandidate');
...
const gameReviewCoachLeadIn = document.getElementById('gameReviewCoachLeadIn');
...
const gameReviewCoachEncouragement = document.getElementById('gameReviewCoachEncouragement');
const btnGameReviewCoach = document.getElementById('btnGameReviewCoach');
const gameReviewCoachProfileControl = document.getElementById('gameReviewCoachProfileControl');
const gameReviewCoachModelProfile = document.getElementById('gameReviewCoachModelProfile');
const gameReviewCoachProfileHint = document.getElementById('gameReviewCoachProfileHint');
const gameReviewCoachStatus = document.getElementById('gameReviewCoachStatus');
...
function refreshHUD() {
  ...
  if (appState === APP_STATE.GAME_RECORD_LIBRARY) {
    ...
    turnText.textContent = gameReviewSession.sourceKind === 'completed' && gameReviewSession.atLast
...
function gameReviewAnnouncement() {
  ...
  if (review.sourceKind === 'completed' && review.atLast && review.snapshot.terminal) {
...
function currentGameReviewTeachingMessage() {
  if (gameReviewTeachingMessageState !== undefined) return gameReviewTeachingMessageState;
  const [message] = deriveGameReviewTeaching(gameReviewEvidenceState);
  return message ?? null;
}

function gameReviewCoachMatchesTeaching(message) {
  if (gameReviewCoachState.status !== 'loading'
    && gameReviewCoachState.status !== 'success') return true;
  if (!message) return false;
  const identity = gameReviewCoachState.identity;
  return !!identity
    && identity.recordId === message.source.recordId
    && identity.ply === message.source.ply
    && identity.positionKey === message.source.positionKey
    && identity.r3aRevision === message.source.r3aRevision
    && identity.ruleId === message.ruleId
    && identity.modelProfile === gameReviewCoachState.modelProfile
    && identity.teachingFingerprint === createTeachingFingerprint(message);
}

function gameReviewCoachCapabilitiesMatch(message, activeRequest = gameReviewCoachCapabilitiesRequest) {
  if (!activeRequest || !message || appState !== APP_STATE.GAME_REVIEW || !gameReviewSession) return false;
  return activeRequest.recordId === message.source.recordId
    && activeRequest.ply === message.source.ply
    && activeRequest.positionKey === message.source.positionKey
    && activeRequest.r3aRevision === message.source.r3aRevision
    && activeRequest.ruleId === message.ruleId
    && activeRequest.teachingFingerprint === createTeachingFingerprint(message)
    && activeRequest.modelProfile === gameReviewCoachState.modelProfile;
}

function resetGameReviewCoachCapabilities() {
  gameReviewCoachCapabilitiesRequest?.controller?.abort();
  gameReviewCoachCapabilitiesRequest = null;
  gameReviewCoachCapabilitiesState = Object.freeze({
    status: gameReviewCoachCapabilitiesLoader ? 'idle' : 'not-required',
    revision: gameReviewCoachCapabilitiesState.revision + 1,
    snapshot: null,
  });
}

function finishGameReviewCoachCapabilities(activeRequest, snapshot) {
  if (activeRequest !== gameReviewCoachCapabilitiesRequest
    || !gameReviewCoachCapabilitiesMatch(currentGameReviewTeachingMessage(), activeRequest)) return false;
  gameReviewCoachCapabilitiesRequest = null;
  gameReviewCoachCapabilitiesState = Object.freeze({
    status: snapshot ? 'ready' : 'error',
    revision: gameReviewCoachCapabilitiesState.revision,
...
function requestGameReviewCoachCapabilities(message) {
  if (!gameReviewCoachCapabilitiesLoader || !message
    || appState !== APP_STATE.GAME_REVIEW || !gameReviewSession
    || gameReviewCoachCapabilitiesState.status !== 'idle'
    || gameReviewCoachCapabilitiesRequest) return false;
  ...
  const activeRequest = Object.freeze({
    ...
    teachingFingerprint: createTeachingFingerprint(message),
    modelProfile: gameReviewCoachState.modelProfile,
    controller,
  });
  gameReviewCoachCapabilitiesRequest = activeRequest;
  gameReviewCoachCapabilitiesState = Object.freeze({
    status: 'loading',
    revision: gameReviewCoachCapabilitiesState.revision + 1,
    snapshot: null,
  ...
  let pending;
  try {
    pending = gameReviewCoachCapabilitiesLoader({ signal: controller.signal });
  } catch {
    finishGameReviewCoachCapabilities(activeRequest, null);
    return true;
  }
  Promise.resolve(pending).then(
    (snapshot) => finishGameReviewCoachCapabilities(activeRequest, snapshot),
    () => finishGameReviewCoachCapabilities(activeRequest, null),
  );
  return true;
}

function gameReviewCoachSelectedProfileAvailable() {
  if (!gameReviewCoachCapabilitiesLoader) return true;
  return gameReviewCoachCapabilitiesState.status === 'ready'
    && !gameReviewCoachUnavailableProfiles.has(gameReviewCoachState.modelProfile)
    && isReviewCoachProfileAvailable(
      gameReviewCoachCapabilitiesState.snapshot,
      gameReviewCoachState.modelProfile,
    );
}

function invalidateGameReviewCoach() {
  gameReviewCoachRequest?.controller?.abort();
  gameReviewCoachRequest = null;
  resetGameReviewCoachCapabilities();
  gameReviewCoachState = invalidateCoachState(gameReviewCoachState);
  gameReviewCoachStatusMessage = '';
  gameReviewCoachLeadIn.classList.add('hidden');
  gameReviewCoachLeadIn.textContent = '';
  gameReviewCoachEncouragement.classList.add('hidden');
  gameReviewCoachEncouragement.textContent = '';
  btnGameReviewCoach.classList.add('hidden');
  gameReviewCoachProfileControl.classList.add('hidden');
  gameReviewCoachModelProfile.disabled = true;
  gameReviewCoachProfileHint.textContent = gameReviewCoachCapabilitiesLoader
    ? '正在確認 staging 教練可用狀態。'
    : '目前為模擬教練，尚未連接模型。';
  btnGameReviewCoach.disabled = false;
  btnGameReviewCoach.setAttribute('aria-disabled', 'false');
  btnGameReviewCoach.setAttribute('aria-busy', 'false');
  gameReviewCoachStatus.textContent = '';
}

function renderGameReviewCoach(message) {
  if (!gameReviewCoachMatchesTeaching(message)) invalidateGameReviewCoach();
  const available = !!gameReviewCoachRequester
    && appState === APP_STATE.GAME_REVIEW
    && !!message
    && gameReviewSession?.sourceKind !== 'live-teaching'
    && gameReviewCoachState.status !== 'disabled';
  if (available) requestGameReviewCoachCapabilities(message);
  const loading = available && gameReviewCoachState.status === 'loading';
  const selectedProfileAvailable = available && gameReviewCoachSelectedProfileAvailable();
  const framing = available && gameReviewCoachState.status === 'success'
    ? gameReviewCoachState.framing
    : null;
  gameReviewCoachLeadIn.classList.toggle('hidden', !framing);
  gameReviewCoachLeadIn.textContent = framing?.leadIn ?? '';
  gameReviewCoachEncouragement.classList.toggle('hidden', !framing);
  gameReviewCoachEncouragement.textContent = framing?.encouragement ?? '';
  btnGameReviewCoach.classList.toggle('hidden', !available);
  gameReviewCoachProfileControl.classList.toggle('hidden', !available);
  gameReviewCoachModelProfile.disabled = !available;
  gameReviewCoachModelProfile.value = gameReviewCoachState.modelProfile;
  for (const option of gameReviewCoachModelProfile.options) {
    option.disabled = !!gameReviewCoachCapabilitiesLoader
      && (gameReviewCoachCapabilitiesState.status !== 'ready'
        || gameReviewCoachUnavailableProfiles.has(option.value)
        || !isReviewCoachProfileAvailable(gameReviewCoachCapabilitiesState.snapshot, option.value));
  }
  if (!gameReviewCoachCapabilitiesLoader) {
    gameReviewCoachProfileHint.textContent = '目前為模擬教練，尚未連接模型。';
  } else if (gameReviewCoachCapabilitiesState.status === 'loading') {
    gameReviewCoachProfileHint.textContent = '正在確認 staging 教練可用狀態。';
  } else if (gameReviewCoachCapabilitiesState.status === 'error') {
    gameReviewCoachProfileHint.textContent = 'AI 教練目前無法使用；已保留你的模型設定。';
  } else if (!selectedProfileAvailable) {
    gameReviewCoachProfileHint.textContent = '所選模型設定目前無法使用；請自行選擇其他設定。';
  } else {
    gameReviewCoachProfileHint.textContent = 'staging 教練已就緒；只會在你按下按鈕後送出請求。';
  }
  // Keep the mounted native button focusable while the request guard blocks reuse.
  btnGameReviewCoach.setAttribute('aria-disabled', String(loading || !selectedProfileAvailable));
  btnGameReviewCoach.setAttribute('aria-busy', String(loading));
  gameReviewCoachStatus.textContent = loading
    ? 'AI 教練整理中…'
    : (available ? gameReviewCoachStatusMessage : '');
}

function renderGameReviewTeaching(evidence, acceptedMessage = gameReviewTeachingMessageState) {
  const [derivedMessage] = acceptedMessage === undefined ? deriveGameReviewTeaching(evidence) : [];
  const message = acceptedMessage === undefined ? derivedMessage : acceptedMessage;
...
function changeGameReviewCoachModelProfile() {
  if (!gameReviewCoachRequester || appState !== APP_STATE.GAME_REVIEW
    || !gameReviewSession || !currentGameReviewTeachingMessage()
    || gameReviewCoachState.status === 'disabled') return false;
  const previous = gameReviewCoachState;
  const next = selectCoachModelProfile(previous, gameReviewCoachModelProfile.value);
  if (next === previous) {
    gameReviewCoachModelProfile.value = previous.modelProfile;
    return false;
  }
  const oldRequest = gameReviewCoachRequest;
  gameReviewCoachRequest = null;
  gameReviewCoachState = next;
  oldRequest?.controller?.abort();
  if (gameReviewCoachCapabilitiesRequest) resetGameReviewCoachCapabilities();
  gameReviewCoachStatusMessage = '';
  if (next.modelProfile !== previous.modelProfile) {
    writeCoachModelProfilePreference(() => globalThis.localStorage, next.modelProfile);
  }
  renderGameReviewCoach(currentGameReviewTeachingMessage());
  return true;
}

function finishGameReviewCoachFailure(activeRequest, error = null) {
  if (activeRequest !== gameReviewCoachRequest) return false;
  gameReviewCoachRequest = null;
  gameReviewCoachState = invalidateCoachState(gameReviewCoachState);
  if (gameReviewCoachCapabilitiesLoader && error?.code === 'profile_unavailable') {
    gameReviewCoachUnavailableProfiles.add(gameReviewCoachState.modelProfile);
    gameReviewCoachStatusMessage = '所選模型設定目前無法使用；請自行選擇其他設定。';
  } else {
    gameReviewCoachStatusMessage = 'AI 教練目前無法整理；原教學提示保持不變。';
  }
  if (appState === APP_STATE.GAME_REVIEW) {
    renderGameReviewCoach(currentGameReviewTeachingMessage());
  }
  return true;
}
...
function handleGameReviewCoachResponse(activeRequest, response) {
  ...
  const message = currentGameReviewTeachingMessage();
  const settled = settleCoachResponse({
    state: gameReviewCoachState,
    currentTeachingMessage: message,
    currentModelProfile: gameReviewCoachState.modelProfile,
    response,
  });
  if (!settled.accepted) return finishGameReviewCoachFailure(activeRequest);
  gameReviewCoachRequest = null;
  gameReviewCoachState = settled.state;
  gameReviewCoachStatusMessage = 'AI 教練已整理完成。';
  renderGameReviewCoach(message);
  return true;
}

function requestGameReviewCoach() {
  const message = currentGameReviewTeachingMessage();
  if (!gameReviewCoachRequester
    || appState !== APP_STATE.GAME_REVIEW
    || !gameReviewSession
    || gameReviewSession.sourceKind === 'live-teaching'
    || !message
    || !gameReviewCoachSelectedProfileAvailable()
    || gameReviewCoachRequest
    || gameReviewCoachState.status === 'loading') return false;
  const requestId = `review-coach-${++gameReviewCoachRequestSequence}`;
  const started = beginCoachRequest({
    state: gameReviewCoachState,
  ...
  gameReviewCoachState = started.state;
  gameReviewCoachRequest = activeRequest;
  gameReviewCoachStatusMessage = '';
  renderGameReviewCoach(message);
  let pending;
  try {
    ...
    finishGameReviewCoachFailure(activeRequest);
    return true;
  }
  Promise.resolve(pending).then(
    (response) => handleGameReviewCoachResponse(activeRequest, response),
    (error) => finishGameReviewCoachFailure(activeRequest, error),
  );
  return true;
}
...
function createGameReviewAiWorker() {
  return new Worker(new URL('./ai-worker.js?v=3a54e4a165', import.meta.url), { type: 'module' });
}
...
function renderGameReview() {
  ...
  if (review.sourceKind === 'live-teaching') {
    appendGameReviewMeta('開始時間', formatGameRecordCompletedAt(review.record.createdAt));
    appendGameReviewMeta('狀態', '進行中的本機對局');
  } else {
...
function enterGameAnalysis(invoker = btnGameReviewAnalyze) {
  if (appState !== APP_STATE.GAME_REVIEW || !gameReviewSession || gameReviewSession.snapshot.terminal) return false;
  try {
    gameAnalysisState = gameReviewSession.sourceKind === 'live-teaching'
      ? createGameLiveReviewAnalysis(gameReviewSession)
      : createGameAnalysis(gameReviewSession.record, gameReviewSession.selectedPly);
  } catch {
...
function openPreparedGameReview(opened, { returnState, invoker, stored = false } = {}) {
...
function openGameReview(record, { returnState, invoker, stored = false } = {}) {
  ...
  if (!openPreparedGameReview(opened, { returnState, invoker, stored })) return false;
  renderGameReview();
  gameReviewHeading.focus({ preventScroll: true });
  return true;
}

function openGameTeachingReviewHandoff(invoker) {
  if (!normalGameActive() || !gameTeachingReviewHandoff) return false;
  const consumed = consumeGameLiveReviewHandoff(gameTeachingReviewHandoff, {
    teachingState: gameTeachingModeState,
    session: normalGameRecordSession,
    history,
  });
  ...
  if (!openPreparedGameReview(consumed.review, {
    returnState: APP_STATE.NORMAL_GAME,
    invoker,
    stored: false,
  })) return false;
  gameReviewAiState = consumed.r3aState;
  gameReviewEvidenceState = consumed.evidence;
  gameReviewTeachingMessageState = consumed.message;
  renderGameReview();
  gameReviewHeading.focus({ preventScroll: true });
  return true;
}
...
function doMove(from, to) {
  ...
  const teachingSource = captureTeachingModeSource(from, to);
  if (teachingSource) invalidateTeachingModeFeedback();
  ...
  tween(340, (k) => {
    ...
    if (cap) {
      ...
      animateCapture(cap, () => {
        ...
        finishMove(nota, captured, gameRecordSessionId, teachingSource);
      });
    } else {
      finishMove(nota, captured, gameRecordSessionId, teachingSource);
    }
...
function finishMove(
  nota,
  captured,
  gameRecordSessionId = normalGameRecordSession?.id,
  teachingSource = null,
) {
...
btnGameReviewCoach.addEventListener('click', requestGameReviewCoach);
gameReviewCoachModelProfile.addEventListener('change', changeGameReviewCoachModelProfile);
...
btnTeachingMode.addEventListener('click', () => {
  setTeachingModeEnabled(!gameTeachingModeState.enabled);
  closeHudMenu();
});
