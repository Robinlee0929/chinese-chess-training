...
- **人機教學模式**：預設關閉，可在上方工具列切換。開啟後只會針對玩家剛走完的一著，在獨立背景 Worker 重用既有 R3A 候選、R3B 事實比較與 R3C-1 固定教學規則；電腦仍照原難度與流程立即回應。沒有適用規則或分析失敗時不顯示評語；悔棋、換局、切換模式或離開對局會清除提示與過期結果。設定只保留於目前分頁，不連網也不寫入瀏覽器儲存。
...
- 「教學模式」預設為關；開啟後，玩家每次完成合法著法可能收到一則由本機規則證實的教學提示，提示不會暫停棋局
...
node game-teaching-mode-test.mjs # 人機教學模式的本機分析、生命週期、零網路與 10 個行為突變閘門
